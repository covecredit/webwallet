...
const NewWalletModal: React.FC<NewWalletModalProps> = ({ onClose }) => {
  ...
  const handleVerify = async () => {
    try {
      ...
      // Validate mnemonic and get seed
      const seed = await validateMnemonic(isRecovery ? userInput : mnemonic);
      setValidatedSeed(seed);

      // Check for existing wallet
      if (walletStorageService.hasStoredSeed()) {
        setShowOverwriteWarning(true);
        return;
      }

      // Show passphrase modal if no passphrase is set
      ...
      setError(error.message || 'Failed to verify mnemonic');
    } finally {
    ...
    <>
      ...
      {showOverwriteWarning && (
        <OverwriteWarningModal
          ...
          onClose={() => setShowOverwriteWarning(false)}
